...
ASYNCIOQUEUE_EXPORT_EXTERN_C sdiAsyncQueueHandle sdiHelperAsyncRepoCreateAsyncioQueueForDSM(
    const int dTypeId,
    const char_T* const dsName,
    const char_T* const logName,
    const char_T* const blockPath,
    const char_T* const sid,
    const int numBlkPathElems,
    const int numDims,
    const int* dims,
    const sdiDimsMode dimsMode,
    const sdiComplexity complexity,
    const sdiSampleTimeContinuity sampleTimeContinuity,
    const char_T* const units,
    const char_T* sampleTimeStr,
    const double simStopTime,
    const int effectiveDecimation,
    const int effectiveMaxPts,
    const bool isHiddenBlk,
    const sdiSharedMemoryType dsmType,
    const int numWriterPaths,
    const int writerPathLength,
    const char_T* writerPathCstr);
...
ASYNCIOQUEUE_EXPORT_EXTERN_C void sdiWriteSignalForDSM(
    sdiAsyncQueueHandle hAsyncQueue,
    const double time,
    const void *const data,
    const int writerIdx);
